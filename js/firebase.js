...
import { initializeApp, getApps, getApp, deleteApp } from "https://www.gstatic.com/firebasejs/12.19.0/firebase-app.js";
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, onAuthStateChanged, signOut, deleteUser, inMemoryPersistence, setPersistence } from "https://www.gstatic.com/firebasejs/12.19.0/firebase-auth.js";
...
function perfil(d){const usuario=texto(d.usuario,80);if(!rolesAdministrables.includes(d.rol))fallo('Rol inválido.');return{usuario,rol:d.rol};}
...
function resumir(s,tipo){const d=s.data();return tipo==='productos'?{...d,id:s.id,revision:revision(d)}:{id:s.id,uid:d.uid||s.id,usuario:d.usuario||d.Usuario||'',email:d.email||'',rol:d.rol||'',proveedor:d.proveedor||'',editable:d.uid===s.id&&rolesAdministrables.includes(d.rol),revision:revision(d)};}
